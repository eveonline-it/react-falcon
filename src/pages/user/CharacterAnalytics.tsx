...
interface SingleItemProps {
  singleData: CharacterAnalyticsData;
}

interface CharacterAnalyticsProps {
  data: CharacterAnalyticsData[];
}
...
const getOptions = (data: CharacterAnalyticsData) => ({
  tooltip: {
    trigger: 'axis',
    formatter: '{b0} : {c0}'
  },
...
const CharacterAnalytics: React.FC<CharacterAnalyticsProps> = ({ data }) => {
  return (
    <Card className="mb-3">
      <Card.Header className="bg-body-tertiary">
        <h5 className="mb-0">Character Analytics</h5>
      </Card.Header>
      <Card.Body>
        <Row className="g-0">
          {data.map(item => (
